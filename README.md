...
### Порядок и цвет карточек в столбце

Тот же каскад, развёрнутый в реальную колонку истории с цветами и границей между Рабочей историей и Формой реализации:

![Структура столбца истории](i/implementation-cascade.svg)

Сверху вниз столбец истории устроен так (цвет — HEX из шаблонов метода, RGB рядом — расчётное значение того же цвета, в скилле отдельно не указано):

| # | Слой | Цвет | HEX | RGB |
|---|---|---|---|---|
| 1 | Цель действия | розовый | `#ED9FC1` | 237, 159, 193 |
| 2 | Носитель действия | жёлтый | `#FFFAB3` | 255, 250, 179 |
| 3 | Ситуация | синий | `#B5BAFD` | 181, 186, 253 |
| 4 | Способ действия | голубой | `#ADCFF6` | 173, 207, 246 |
| 5 | Объекты оперирования | серый | `#DBDCDD` | 219, 220, 221 |
| — | — черта — | | | |
| 6 | Формы реализации | жёлтый (тёмный) | `#F5D634` | 245, 214, 52 |
| 7 | Структура интерфейсных блоков | зелёный | `#B8CE4F` | 184, 206, 79 |

Порядок в столбце — это порядок влияния элементов друг на друга (Цель определяет всё нижестоящее), а не порядок, в котором их читают вслух: вслух карточки читаются в последовательности Н-С-Д-О-Ц.
...
- **Нумерация** — сквозной номер истории (например, СР-12 или 3.12), по которому на неё можно сослаться в обсуждении.
...
### Как проверить готовность истории
